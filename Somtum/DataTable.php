<?php
/**
 * @filesource Somtum/DataTable.php
 *
 ...
 * @since 1.0
 */
class DataTable extends \Somtum\Base
{
    /**
     * id ของตาราง.
     *
    ...
    private $model;
    /**
     * เก็บ where ของ query.
     *
     * @var array
     */
    private $coditions = array();
    /**
    ...
    public $sort = null;
    /**
     * คอลัมน์ที่สามารถใช้เรียงลำดับได้.
     *
     * @var array
     */
    ...
    public function __construct($param)
    {
        ...
        if ($this->perPage !== null) {
            $count = (int) self::$request->request('count');
        ...
        if (is_string($this->model)) {
            $this->model = createClass($this->model);
        }
        // รับค่าการเรียงลำดับถ้ามีการกำหนด sort ไว้
        $this->sort = self::$request->request('sort', $this->sort);
        if (!empty($this->sort)) {
    ...
    public function render()
    {
        ...
        foreach ($this->filters as $key => $items) {
            ...
            if (!empty($items['options']) && isset($items['value']) && $items['value'] !== $items['default'] && in_array($items['value'], array_keys($items['options']), true)) {
                $this->coditions[$key] = $items['value'];
            }
        }
        // ปุ่ม Go
        if (!empty($form)) {
            $form[] = '<fieldset class=go>';
            $form[] = '<button type=submit class="button go">Go</button>';
        ...
        if (!empty($this->searchColumns)) {
                ...
                if ($this->autoSearch) {
                    if (isset($this->model)) {
                        ...
                        foreach ($this->searchColumns as $key) {
                            $sh[$key] = '`'.$key.'` LIKE :search';
                        }
                        $this->coditions['('.implode(' OR ', $sh).')'] = "%$search%";
                    }
                }
            ...
            $form[] = '<label accesskey=f><input type=text name=search value="'.$search.'" placeholder="ค้นหา"></label>';
        ...
        if (isset($this->model)) {
            // จำนวนข้อมูลทั้งหมด
            $count = $this->model->count($this->coditions);
        ...
        if ($this->perPage > 0) {
            // หน้าที่เลือก
            $page = max(1, (int) self::$request->request('page', 1));
        ...
        if ($this->showCaption) {
            if (empty($search)) {
                $caption = 'ทั้งหมด :count รายการ, แสดงรายการที่ :start ถึง :end, หน้าที่ :page จาก :total หน้า';
            } else {
                $caption = 'ค้นหา <strong>:search</strong> พบ :count รายการ, แสดงรายการที่ :start ถึง :end, หน้าที่ :page จาก :total หน้า';
            }
        ...
        if (isset($this->model)) {
            // query ข้อมูล
            $this->datas = $this->model->execute($this->coditions, $this->perPage, $start, $this->sort);
            // รายการสุดท้าย
            $end = $this->perPage + 1;
            // รายการแรก
            $start = -1;
